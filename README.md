...
| **Telegram:** Searcharr waits after a host restart | Expected while Sonarr/Radarr initialize. `docker logs searcharr` shows the readiness gate waiting, then launching Searcharr after both authenticated APIs respond. This gate runs even when Docker's restart policy bypasses Compose dependency ordering. |
| **Telegram:** Searcharr can't authenticate to Sonarr/Radarr | Re-run setup with `-Telegram`/`--telegram` after Sonarr/Radarr regenerate their API keys. Setup rewrites `settings.py` and force-recreates Searcharr so the running bot cannot retain stale credentials. The healthcheck also fails if the mounted settings change without a restart. |
